const listAccounts = [
  ...
  const newListAccount = listAccounts.map(function(value, index) {
      if (value.isActive === false)
        return `username: ${value.username}, balance: ${value.balance}, type: ${value.type}, isActive: 'true'`
    else return `username: ${value.username}, balance: ${value.balance}, type: ${value.type}, isActive: 'normal'`
  });
  
 console.log(newListAccount);
  
            // tính tổng balance
  const totalBalance = listAccounts.reduce((accumulator, curentValue) => accumulator + curentValue.balance, 0)
  ...
  const statisticsByAge = listPersons.reduce(
      (accumulator, curentValue) => {
          if (curentValue.age <= 18) accumulator.treCon++;
          else if (curentValue.age > 18 && curentValue.age <= 30) accumulator.thanhNien++;
          else accumulator.nguoiGia++;
          return accumulator
      }, {
          treCon: 0,
          thanhNien: 0,
